...
void instant_event(const char* category, const char* name, const uint64_t* track_id, const PerfettoEventArg* args, size_t arg_count) {
	...
	if (category) {
		perfetto::DynamicCategory category_name{category};
		if (track_id) {
			TRACE_EVENT_INSTANT(category_name, name_str, perfetto::Track(*track_id), set_props);
		} else {
			TRACE_EVENT_INSTANT(category_name, name_str, set_props);
		}
	} else {
		if (track_id) {
			TRACE_EVENT_INSTANT("default", name_str, perfetto::Track(*track_id), set_props);
		} else {
			TRACE_EVENT_INSTANT("default", name_str, set_props);
		}
	}
}
